...
import {
  fetchProcesses,
  // ⛔ ya no se usan aquí para evitar duplicados:
  // createProcess,
  // updateProcess,
  deleteProcess,
} from "../data/processes"; 
...
const ProcessesPage = () => {
  ...
  // evita doble onSubmit por doble click
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      console.error("[ProcessesPage] fetch error:", e);
      setErr("No se pudieron cargar los procesos.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const procesosFiltrados = useMemo(() => {
    ...
    return processes.filter((proceso) => {
      const nombre = (proceso?.nombre ?? "").toLowerCase();
      const descripcion =
        (proceso?.descripcion ??
          proceso?.producto?.description ??
          "")
          .toLowerCase();
  ...
  const procesosActuales = procesosFiltrados.slice(
    indicePrimerProcess,
    indiceUltimoProcess
  );
  ...
  const cerrarFormulario = () => {
    ...
    setProcesoAEditar(null);
    submittingRef.current = false; // reset anti-doble submit
  };
  ...
  // ⛔ NO llamamos a createProcess/updateProcess aquí.
  // ProcessForm se encarga de la API; aquí solo recargamos y cerramos.
  const handleCrear = async (_payloadOrSaved) => {
    try {
      ...
      console.error("[ProcessesPage] handleCrear error:", e);
      alert("Ocurrió un problema al refrescar la lista.");
    } finally {
      submittingRef.current = false;
    }
  };

  const handleActualizar = async (_payloadOrSaved) => {
    try {
      if (submittingRef.current) return;
      submittingRef.current = true;
      ...
      console.error("[ProcessesPage] handleActualizar error:", e);
      alert("Ocurrió un problema al refrescar la lista.");
    } finally {
      submittingRef.current = false;
    }
  ...
  const eliminarProceso = async (id) => {
    ...
    try {
      ...
      console.error("[ProcessesPage] delete error:", e);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-600">
              Gestiona y revisa todos los procesos de producción
            </p>
          </div>
        ...
        {!loading && !err && procesosFiltrados.length > 0 ? (
          <>
      ...
      {mostrarFormulario && (
        <ProcessForm
          ...
          onSubmit={procesoAEditar ? handleActualizar : handleCrear}
