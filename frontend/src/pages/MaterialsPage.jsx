...
import { materialesPrueba } from '../data/materialsData';
...
const MaterialsPage = () => {
  ...
  // Filtrar materiales por busqueda y tipo
  ...
  // Editar material - Abre el formulario en modo edicion
  ...
  // Abrir formulario en modo creacion
  ...
  const agregarMaterial = (nuevoMaterial) => {
    const nuevoMaterialConId = {
      ...nuevoMaterial,
      id: Math.max(...materiales.map(m => m.id)) + 1,
      tipo: nuevoMaterial.etiqueta
    };
    
    setMateriales([...materiales, nuevoMaterialConId]);
    setMostrarFormulario(false);
    console.log("Material agregado:", nuevoMaterialConId);
  };

  // Actualizar material existente
  const actualizarMaterial = (materialActualizado) => {
    const materialesActualizados = materiales.map(material => 
      material.id === materialAEditar.id 
        ? { ...materialActualizado, id: materialAEditar.id, tipo: materialActualizado.etiqueta }
        : material
    );
    ...
    // Si estabamos viendo detalles, actualizamos el material seleccionado
    if (vistaDetalle && materialSeleccionado && materialSeleccionado.id === materialAEditar.id) {
      setMaterialSeleccionado({ ...materialActualizado, id: materialAEditar.id, tipo: materialActualizado.etiqueta });
    }
